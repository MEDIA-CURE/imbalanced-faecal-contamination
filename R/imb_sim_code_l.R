#############################################
#Datos SIMULADOS LINEALES Para umbalanced#
#############################################
...
ERRORESl<-matrix(NaN, ncol=9, nrow=nsim) # modelos x simulaciones
AUCsl<-matrix(NaN, ncol=9, nrow=nsim) # Matrix con las Areas bajo la curva
TP_MATl<-matrix(NaN, ncol=9, nrow=nsim) # Matriz con los true positive calculados por TP_fun
FP_MATl<-matrix(NaN, ncol=9, nrow=nsim) # Matriz con los true positive calculados por FP_fun
...
for(k in 1:nsim){
  ...
  print(k)
  n=nrow(simul_data_l)
  s=ech.tst(simul_data_l[,1],prop=1/3)
  ...
  # TRUE POSITIVE RATE
  truePositive_l= cbind(LDA=TP_fun(yobstst,evdis)$TPR, SVM=TP_fun(yobstst,evsvm)$TPR, LR=TP_fun(yobstst,evglm)$TPR,
...
tabla_lineal=rbind(Acctab,AUCtab,TPRtab,FPRtab)


fic = paste("reslineal.txt",sep="")
...
dump(paste("tabla_lineal"),file=fic,append=T)

xtable(tabla_lineal,caption = "Tabla Lineal")





#pdf("Figure_X_ErrorEvalLineal.pdf", height=7, width=8)
